...
@pytest.fixture()
def pharma_clear_client(
    algorand_client: AlgorandClient, deployer: SigningAccount
) -> PharmaClearClient:
    factory = algorand_client.client.get_typed_app_factory(
        PharmaClearFactory, default_sender=deployer.address
    )
...
def test_says_hello(pharma_clear_client: PharmaClearClient) -> None:
    result = pharma_clear_client.send.hello(args=("World",))
    assert result.abi_return == "Hello, World"


def test_simulate_says_hello_with_correct_budget_consumed(
    pharma_clear_client: PharmaClearClient,
) -> None:
    result = (
        pharma_clear_client.new_group()
        .hello(args=("World",))
        .hello(args=("Jane",))
        .simulate()
    )
